...
- **`activity_log`: hier weicht dieser Plan bewusst von iOS ab.** iOS schreibt für eine Review-Ersetzung **keine** Verlaufszeile. Android hat für exakt dieselbe Operation schon einen Schreibpfad (`MachineAnalysisRepository.logProductSwap`, Action `analysis_swap`), und ein Produktwechsel ohne Audit-Zeile ist in einem Mehrbenutzer-Betrieb eine Lücke. **Entscheidung: wir schreiben eine Zeile.** **Korrektur während der Umsetzung:** die erste Planfassung wollte dafür eine eigene Action `refill_review_swap`. Ein Blick eine Ebene tiefer zeigt, dass eine neue Action **nirgends** gerendert wird: `MachineAnalysisRepository.logProductSwap` schreibt die Action `product_swapped` und unterscheidet die Herkunft über `metadata.source = "analysis_swap"`, und der gemeinsame PWA-Renderer (`management-frontend/app/lib/activityDescriptor.ts`) verzweigt über die **Action** und beschriftet die Quelle nur. Also: Action `product_swapped`, `source = "refill_review"`. Damit erscheint die Zeile in der PWA sofort und bleibt trotzdem unterscheidbar — genau die Eigenschaft, die der Plan wollte. Für diesen Quellwert fehlt der PWA noch ein Label; sie rendert die Zeile dann ohne Quellen-Chip, bricht aber nicht. Der Metadaten-Vertrag gilt wie immer: keine bestehende Schlüssel-Typänderung, und `ActivityFeedBuilder` ignoriert unbekannte Actions still (`mapNotNull`), also bricht nichts, solange die Zeile nur additiv ist. Rendering im Feed ist **nicht** Teil dieses Plans.
...
4. Ein Ersatz wird gesetzt und angewandt: das Fach trägt danach das neue Produkt mit Bestand **0**, taucht im Pack-Schritt mit vollem Defizit auf, und es existiert eine `product_swapped`-Zeile mit `source = "refill_review"` (auf Android nicht gerendert, in der PWA unter `/history` sehr wohl — dort prüfen).
