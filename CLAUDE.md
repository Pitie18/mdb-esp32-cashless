...
- `useMachinePrint()` – data + QR rendering for the printable machine posters (`/machines/[id]/print`): resolves `machine.x ?? company.x` contact data, builds the QR targets, renders them as **SVG** (not data URLs — visibly sharper at 5 cm on paper), and writes the `poster_printed` activity entry. The pure logic lives in `app/lib/printSheet.ts` and is unit-tested; motifs are registered in `app/lib/printMotifs.ts`
...
- `/machines/[id]/print` – Printable contact signage for a machine: four A4/A5/A6 poster motifs plus two 90×50 mm sticker motifs (8-up on A4 with cut marks), QR codes to the public machine page / `tel:` / WhatsApp / the fault-report form, per-sheet language, free text, and batch selection across machines. Prints via `window.print()` with `@page` + `print-color-adjust: exact`. Its own route rather than a modal so the app chrome never has to be hidden. The QR origin comes from `SITE_URL` (`runtimeConfig.public.siteUrl`); a LAN or localhost origin raises a blocking-looking warning, because a wrong QR on paper is permanent
